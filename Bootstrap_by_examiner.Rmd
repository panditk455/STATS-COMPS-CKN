...
# unique examiners
examiners <- unique(responses$AnonID)

# pre-split row indices by examiner (fast & memory efficient)
idx_by_examiner <- split(seq_len(nrow(responses)), responses$AnonID)

# store bootstrap indices
boot_idx_AnonID <- vector("list", B)

for (b in 1:B) {
  # sample examiners with replacement
  sampled_examiners <- sample(examiners, length(examiners), replace = TRUE)
  
  # collect all rows belonging to sampled examiners
  boot_idx_AnonID[[b]] <- unlist(idx_by_examiner[sampled_examiners], use.names = FALSE)
}

# save to disk
saveRDS(boot_idx_AnonID, "boot_idx_by_AnonID.rds")

boot_idx_AnonID <- readRDS("boot_idx_by_AnonID.rds")

results <- vector("list", length(boot_idx_AnonID))
```
...
```{r}
out <- vector("list", length(boot_idx_AnonID))

for (b in seq_along(boot_idx_AnonID)) {
  
  # Step 1: get bootstrap dataset

  # Step 2 (LATER): apply Hawthorne effect
  # boot_dat <- apply_hawthorne(boot_dat)
    boot_dat <- responses[boot_idx_AnonID[[b]], ]
...
quantile(vals2, c(.025, .975), na.rm = TRUE) 
```
